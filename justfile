...
# Run all MoonBit tests (unit, seeded properties, simulations, bridge)
test:
  moon test
...
# Build the JS bridge consumed by examples/cf-room
build-js:
...
simulate seed="1":
  moon test src/mmo/simulation -v

# Install the Cloudflare host
install-cf-room:
  pnpm --dir examples/cf-room install --frozen-lockfile

# Type-check the Cloudflare host (builds the JS bridge first)
check-cf-room:
  pnpm --dir examples/cf-room typecheck

# Run the workerd Durable Object tests
test-cf-room:
  pnpm --dir examples/cf-room test

# Validate the Worker deploy bundle without mutating Cloudflare
build-cf-room:
  pnpm --dir examples/cf-room deploy:dry

# Start the Worker locally
dev-cf-room:
  pnpm --dir examples/cf-room dev
